...
async function processJob(
  ...
  try {
    const response = await fetch(process.env.PDF_API_URL ?? 'http://pdf.empresor.com.br/pdf', {
      method: 'POST',
      headers: {
...
export async function POST(
  ...
  await db('pdf_jobs').insert({ id: jobId, quote_id: params.quoteId, status: 'pending' });
